    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          ...
          <div class="card-sub">活跃的会话密钥和每个会话的覆盖设置。</div>
        </div>
          ...
          ${thinkLevels.map(
            (level) =>
              html`<option value=${level} ?selected=${thinking === level}>
                ${level === "off" ? "关闭" : level === "minimal" ? "最小" : level === "low" ? "低" : level === "medium" ? "中等" : level === "high" ? "高" : level === "xhigh" ? "极高" : level === "on" ? "开启" : level || "继承"}
              </option>`,
          )}
        </select>
      </div>
          ...
          ${reasoningLevels.map(
            (level) =>
              html`<option value=${level} ?selected=${reasoning === level}>
                ${level === "off" ? "关闭" : level === "on" ? "开启" : level === "stream" ? "流式" : level || "继承"}
              </option>`,
          )}
        </select>
      </div>
